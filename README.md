...
Each experiment needs a `data/` directory, which has been compressed and uploaded to [Google Drive](). To get started, download the `.tar` files from [Google Drive]() and extract them in the matching experiment foder. For example, the `01_purity_levels_experiment/exclude_normal_epithelial.tar` file has to be extracted in the `01_purity_levels_experiment/exclude_normal_epithelial` directory. This will provide you with a data folder with this content:
...
- Download the `Scaden.tar` file on [Google Drive]() and extract it
- Place the model folders in their matching experiment's `data/scaden/` directories
- Ensure there are no other train model folders in the same directory
- Comment out `scaden process` and `scaden process` in the matching `.pbs` run scripts before running Scaden 
